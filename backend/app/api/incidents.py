...
@router.get("/jira")
async def list_jira_incidents(max_results: int = Query(20)):
    """
    Fetch live Jira issues and return them shaped as Kanban cards.
    Returns empty list (not an error) if Jira is not configured.
    """
    if not jira_client.is_configured():
        return []

    raw_issues = jira_client.list_recent_issues(max_results=max_results)
    ...
    for issue in raw_issues:
        ...
        cards.append({
            ...
            "status": status,
            "priority": priority,
            "severity": priority,          # map priority → severity for display
            "service": "Jira",
            "assigned_to": issue.get("assigned_to") or "Unassigned",
            "created_at": issue.get("created_at") or "",
            "updated_at": issue.get("created_at") or "",
            ...
            "duration_text": issue.get("duration_text") or "Active",
            "mttr_minutes": issue.get("mttr_minutes"),
